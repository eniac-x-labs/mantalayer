...
import "../events/IDelegationManagerEvents.sol";
import "../utils/MantaLayerUnitTestSetup.sol";
 ...
 */
contract DelegationManagerUnitTests is MantaLayerUnitTestSetup, IDelegationManagerEvents {
    // Mocks
    ...
    address defaultStaker = vm.addr(uint256(123_456_789));
    ...
    IStrategyBase public constant beaconChainETHStrategy = IStrategyBase(0xbeaC0eeEeeeeEEeEeEEEEeeEEeEeeeEeeEEBEaC0);
    ...
    mapping(IStrategyBase => uint256) public delegatedSharesBefore;

    function setUp() public virtual override {
        // Setup
        MantaLayerUnitTestSetup.setUp();
        // Deploy DelegationManager implmentation and proxy
        initializeStrategiesToSetDelayBlocks = new IStrategyBase[](0);
        ...
        strategyMock = StrategyBase(address(strategyBaseProxyInstance));
        ...
        strategyBaseProxyAdmin.upgradeAndCall(
            ITransparentUpgradeableProxy(payable(address(strategyMock))),
     ...
     */
    function _deployAndDepositIntoStrategies(address staker, uint256[] memory sharesAmounts)
        internal
        returns (IStrategyBase[] memory)
    {
        uint256 numStrats = sharesAmounts.length;
        IStrategyBase[] memory strategies = new IStrategyBase[](numStrats);
    ...
    function _getStakerSignature(uint256 _stakerPrivateKey, address operator, uint256 expiry)
        internal
        view
        returns (ISignatureUtils.SignatureWithExpiry memory stakerSignatureAndExpiry)
    {
    ...
    function _filterOperatorDetails(address operator, IDelegationManager.OperatorDetails memory operatorDetails)
        internal
        view
    {
     ...
     */
    function _fuzzWithdrawalAmounts(uint256[] memory depositAmounts) internal pure returns (uint256[] memory) {
        uint256[] memory withdrawalAmounts = new uint256[](depositAmounts.length);
        for (uint256 i = 0; i < depositAmounts.length; i++) {
            vm.assume(depositAmounts[i] > 0);
    ...
    {
        IStrategyBase[] memory strategyArray = new IStrategyBase[](1);
    ...
    function _setUpCompleteQueuedWithdrawalSingleStrat(
        ...
        uint256 withdrawalAmount
    ) internal returns (IDelegationManager.Withdrawal memory, IERC20, bytes32) {
        uint256[] memory depositAmounts = new uint256[](1);
        depositAmounts[0] = depositAmount;
        IStrategyBase[] memory strategies = _deployAndDepositIntoStrategies(staker, depositAmounts);
        (
            IDelegationManager.QueuedWithdrawalParams[] memory queuedWithdrawalParams,
        ...
        IERC20 token = strategies[0].underlyingToken();
        return (withdrawal, token, withdrawalRoot);
    }
    ...
    function _setUpCompleteQueuedWithdrawalBeaconStrat(
        ...
        uint256[] memory depositAmounts = new uint256[](1);
        depositAmounts[0] = depositAmount;
        IStrategyBase[] memory strategies = new IStrategyBase[](1);
...
contract DelegationManagerUnitTests_Initialization_Setters is DelegationManagerUnitTests {
    function test_initialization() public view {
        ...
        assertEq(
            ...
            address(mantaLayerPauserReg),
            "constructor / initializer incorrect, mantaLayerPauserReg set wrong"
        );
        assertEq(delegationManager.owner(), executorMultisig, "constructor / initializer incorrect, owner set wrong");
    ...
    function testFuzz_setMinWithdrawalDelayBlocks_revert_notOwner(address invalidCaller)
        public
        filterFuzzedAddressInputs(invalidCaller)
    {
        vm.assume(invalidCaller != delegationManager.owner());
        vm.prank(invalidCaller);
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, invalidCaller));
    ...
    function testFuzz_setMinWithdrawalDelayBlocks_revert_tooLarge(uint256 newMinWithdrawalDelayBlocks) external {
        ...
        vm.assume(newMinWithdrawalDelayBlocks > delegationManager.MAX_WITHDRAWAL_DELAY_BLOCKS());
        vm.prank(executorMultisig);
    ...
    function testFuzz_initialize_Revert_WhenWithdrawalDelayBlocksTooLarge(
        ...
        vm.assume(withdrawalDelayBlocks.length > 0);
        uint256 numStrats = withdrawalDelayBlocks.length;
        IStrategyBase[] memory strategiesToSetDelayBlocks = new IStrategyBase[](numStrats);
        for (uint256 i = 0; i < numStrats; i++) {
            strategiesToSetDelayBlocks[i] = IStrategyBase(address(uint160(uint256(keccak256(abi.encode(strategyMock, i))))));
        }
        ...
        // Deploy DelegationManager implmentation and proxy
        DelegationManager _delegationManagerImplementation = new DelegationManager(strategyManager);
        TransparentUpgradeableProxy _delegationManagerProxyInstance =
            new TransparentUpgradeableProxy(address(emptyContract), executorMultisig, "");
        DelegationManager _delegationManager = DelegationManager(address(_delegationManagerProxyInstance));
        ProxyAdmin _delegationManagerProxyAdmin =
            ProxyAdmin(getProxyAdminAddress(address(_delegationManagerProxyInstance)));
        vm.expectRevert(
            "DelegationManager._setStrategyWithdrawalDelayBlocks: _withdrawalDelayBlocks cannot be > MAX_WITHDRAWAL_DELAY_BLOCKS"
        );
        vm.prank(executorMultisig);
        _delegationManagerProxyAdmin.upgradeAndCall(
...
contract DelegationManagerUnitTests_RegisterModifyOperator is DelegationManagerUnitTests {
    ...
    function testFuzz_registerAsOperator(
        ...
        vm.expectEmit(true, true, true, true, address(delegationManager));
        emit OperatorNodeUrlUpdated(operator, nodeUrl);

        vm.prank(operator);
        delegationManager.registerAsOperator(operatorDetails, nodeUrl);
    ...
    function testFuzz_registerAsOperator_cannotRegisterWhileDelegated(
        ...
        vm.assume(staker != defaultOperator);
    ...
    function testFuzz_modifyOperatorParameters(
        ...
        } else if (modifiedOperatorDetails.stakerOptOutWindowBlocks >= initialOperatorDetails.stakerOptOutWindowBlocks)
        {
            ...
            vm.expectRevert("DelegationManager._setOperatorDetails: stakerOptOutWindowBlocks cannot be decreased");
            delegationManager.modifyOperatorDetails(modifiedOperatorDetails);
        }
    ...
    // @notice Tests that an address which is not an operator cannot successfully call `updateOperatorNodeUrl`.
    function test_updateOperatorNodeUrl_notRegistered() public {
        assertFalse(delegationManager.isOperator(defaultOperator), "bad test setup");

        vm.prank(defaultOperator);
        vm.expectRevert("DelegationManager.updateOperatorNodeUrl: caller must be an operator");
        delegationManager.updateOperatorNodeUrl(emptyStringForMetadataURI);
    }
     ...
     */
    function testFuzz_updateOperatorNodeUrl_revert_notOperator(
        string memory nodeUrl
    ) public {
        vm.expectRevert("DelegationManager.updateOperatorNodeUrl: caller must be an operator");
        delegationManager.updateOperatorNodeUrl(nodeUrl);
    }

    // @notice Tests that an operator who calls `updateOperatorNodeUrl` will correctly see an `OperatorNodeUrlUpdated` event emitted with their input
    function testFuzz_updateOperatorNodeUrl(string memory nodeUrl) public {
        // register *this contract* as an operator
        _registerOperatorWithBaseDetails(defaultOperator);

        // call `updateOperatorNodeUrl` and check for event
        vm.prank(defaultOperator);
        vm.expectEmit(true, true, true, true, address(delegationManager));
        emit OperatorNodeUrlUpdated(defaultOperator, nodeUrl);
        delegationManager.updateOperatorNodeUrl(nodeUrl);
    }
}
...
contract DelegationManagerUnitTests_delegateTo is DelegationManagerUnitTests {
    ...
    function testFuzz_Revert_WhenDelegateWhileDelegated(
        ...
        vm.startPrank(staker);
        vm.expectRevert("DelegationManager.delegateTo: staker is already actively delegated");
        delegationManager.delegateTo(operator, approverSignatureAndExpiry, salt);
        vm.stopPrank();
    }

    // @notice Verifies that `staker` cannot delegate to an unregistered `operator`
    function testFuzz_Revert_WhenDelegateToUnregisteredOperator(address staker, address operator)
        public
    ...
    function testFuzz_OperatorWhoAcceptsAllStakers_StrategyManagerShares(
        ...
        IStrategyBase[] memory strategiesToReturn = new IStrategyBase[](1);
    ...
    function testFuzz_Revert_WhenOperatorWhoRequiresECDSASignature_ExpiredDelegationApproverSignature(
        ...
        // delegate from the `staker` to the operator
        vm.startPrank(staker);
        vm.expectRevert("DelegationManager._delegate: approver signature expired");
        delegationManager.delegateTo(defaultOperator, approverSignatureAndExpiry, salt);
        vm.stopPrank();
    }
    ...
    function testFuzz_Revert_WhenOperatorWhoRequiresECDSASignature_PreviouslyUsedSalt(
        ...
        vm.assume(staker != defaultOperator);
        vm.assume(staker != defaultApprover);
        ...
        vm.expectRevert("DelegationManager._delegate: approverSalt already spent");
        delegationManager.delegateTo(defaultOperator, approverSignatureAndExpiry, salt);
        vm.stopPrank();
    }
     ...
     */
    function testFuzz_Revert_WhenOperatorWhoRequiresECDSASignature_WithBadSignature(address staker, uint256 expiry)
        public
        filterFuzzedAddressInputs(staker)
    {
        // filter to only valid `expiry` values
        ...
        // filter inputs, since this will fail when the staker is already registered as an operator
        vm.assume(staker != defaultOperator && staker != defaultApprover);
     ...
     * Reverts if the ‘operator’ is not actually registered as an operator
     */
    function testFuzz_OperatorWhoRequiresECDSASignature(address staker, bytes32 salt, uint256 expiry)
        public
        filterFuzzedAddressInputs(staker)
    {
        // filter to only valid `expiry` values
    ...
    function testFuzz_Revert_WhenOperatorWhoRequiresEIP1271Signature_PreviouslyUsedSalt(
        ...
        vm.assume(staker != address(wallet) && staker != defaultOperator);
     ...
     */
    function testFuzz_Revert_WhenOperatorWhoRequiresEIP1271Signature_NonCompliantWallet(address staker, uint256 expiry)
        public
        filterFuzzedAddressInputs(staker)
    {
        // filter to only valid `expiry` values
        ...
        vm.assume(staker != address(wallet));
    ...
    function testFuzz_Revert_WhenOperatorWhoRequiresEIP1271Signature_IsValidSignatureFails(
        ...
        vm.expectRevert("EIP1271SignatureUtils.checkSignature_EIP1271: ERC1271 signature verification failed");
        delegationManager.delegateTo(defaultOperator, approverSignatureAndExpiry, emptySalt);
        vm.stopPrank();
    }
     ...
     * Reverts if the ‘operator’ is not actually registered as an operator
     */
    function testFuzz_OperatorWhoRequiresEIP1271Signature(address staker, bytes32 salt, uint256 expiry)
        public
        filterFuzzedAddressInputs(staker)
    {
        // filter to only valid `expiry` values
...
contract DelegationManagerUnitTests_delegateToBySignature is DelegationManagerUnitTests {
    ...
    function test_Revert_Staker_WhenAlreadyDelegated() public {
        ...
        vm.startPrank(caller);
        vm.expectRevert("DelegationManager.delegateToBySignature: staker is already actively delegated");
        // use an empty approver signature input since none is needed / the input is unchecked
        ISignatureUtils.SignatureWithExpiry memory approverSignatureAndExpiry;
        delegationManager.delegateToBySignature(
            staker, defaultOperator, stakerSignatureAndExpiry, approverSignatureAndExpiry, emptySalt
    ...
    function test_Revert_EOAStaker_OperatorNotRegistered() public {
        ...
        vm.startPrank(caller);
        vm.expectRevert("DelegationManager.delegateToBySignature: operator is not registered in EigenLayer");
        // use an empty approver signature input since none is needed / the input is unchecked
        ISignatureUtils.SignatureWithExpiry memory approverSignatureAndExpiry;
        delegationManager.delegateToBySignature(
            staker, defaultOperator, stakerSignatureAndExpiry, approverSignatureAndExpiry, emptySalt
    ...
    function testFuzz_Revert_WhenDelegationApproverSignatureExpired(
        ...
        vm.assume(caller != defaultOperator);
    ...
    function testFuzz_ERC1271Staker_OperatorWhoAcceptsAllStakers(
        ...
        defaultStaker = address(ERC1271WalletMock(vm.addr(stakerPrivateKey)));
        testFuzz_EOAStaker_OperatorWhoAcceptsAllStakers(caller, expiry, shares);
    }
...
contract DelegationManagerUnitTests_ShareAdjustment is DelegationManagerUnitTests {
    ...
    function testFuzz_increaseDelegatedShares_revert_invalidCaller(address invalidCaller, uint256 shares)
        public
        filterFuzzedAddressInputs(invalidCaller)
    {
        vm.assume(invalidCaller != address(strategyManagerMock));
        vm.assume(invalidCaller != address(strategyManagerProxyAdmin));
    ...
    function testFuzz_increaseDelegatedShares_noop(address staker) public {
        ...
        vm.prank(address(strategyManagerMock));
    ...
    function testFuzz_decreaseDelegatedShares_revert_invalidCaller(address invalidCaller, uint256 shares)
        public
        filterFuzzedAddressInputs(invalidCaller)
    {
        vm.assume(invalidCaller != address(strategyManagerMock));
    ...
    function testFuzz_decreaseDelegatedShares(
        ...
        vm.assume(strategies.length <= 32);
        vm.assume(staker != defaultOperator);
...
contract DelegationManagerUnitTests_Undelegate is DelegationManagerUnitTests {
    ...
    function testFuzz_undelegate_revert_notDelgated(address undelegatedStaker)
        public
        filterFuzzedAddressInputs(undelegatedStaker)
    {
        vm.assume(undelegatedStaker != defaultOperator);
        assertFalse(delegationManager.isDelegated(undelegatedStaker), "bad test setup");

        vm.prank(undelegatedStaker);
    ...
    function test_undelegate_revert_zeroAddress() public {
        ...
        vm.prank(address(0));
        vm.expectRevert("DelegationManager.undelegate: cannot undelegate zero address");
     ...
     */
    function testFuzz_undelegate_revert_invalidCaller(address invalidCaller)
        public
        filterFuzzedAddressInputs(invalidCaller)
    {
        address staker = address(0x123);
        // filter out addresses that are actually allowed to call the function
        vm.assume(invalidCaller != staker);
        vm.assume(invalidCaller != defaultOperator);
        vm.assume(invalidCaller != defaultApprover);
...
contract DelegationManagerUnitTests_queueWithdrawals is DelegationManagerUnitTests {
    ...
    function test_Revert_WhenQueueWithdrawalParamsLengthMismatch() public {
        ...
        vm.expectRevert("DelegationManager.queueWithdrawal: input length mismatch");
        delegationManager.queueWithdrawals(queuedWithdrawalParams);
    }

    function test_Revert_WhenNotStakerWithdrawer(address withdrawer) public {
        vm.assume(withdrawer != defaultStaker);
    ...
    function test_Revert_WhenEmptyStrategiesArray() public {
        IStrategyBase[] memory strategyArray = new IStrategyBase[](0);
        ...
        vm.expectRevert("DelegationManager._removeSharesAndQueueWithdrawal: strategies cannot be empty");
        delegationManager.queueWithdrawals(queuedWithdrawalParams);
    }
     ...
     */
    function testFuzz_queueWithdrawal_SingleStrat(address staker, uint256 depositAmount, uint256 withdrawalAmount)
        public
        filterFuzzedAddressInputs(staker)
    {
        vm.assume(staker != defaultOperator);
        vm.assume(withdrawalAmount > 0 && withdrawalAmount <= depositAmount);
        ...
        IStrategyBase[] memory strategies = _deployAndDepositIntoStrategies(staker, sharesAmounts);
        _registerOperatorWithBaseDetails(defaultOperator);
        _delegateToOperatorWhoAcceptsAllStakers(staker, defaultOperator);
        (
            IDelegationManager.QueuedWithdrawalParams[] memory queuedWithdrawalParams,
     ...
     * - Checks that event was emitted with correct withdrawalRoot and withdrawal
     */
    function testFuzz_queueWithdrawal_MultipleStrats(address staker, uint256[] memory depositAmounts)
        public
        filterFuzzedAddressInputs(staker)
    {
        vm.assume(staker != defaultOperator);
        vm.assume(depositAmounts.length > 0 && depositAmounts.length <= 32);
    ...
    function testFuzz_queueWithdrawal_Revert_WhenThirdPartyTransfersForbidden(
        ...
        vm.assume(staker != withdrawer && staker != defaultOperator);
        vm.assume(depositAmounts.length > 0 && depositAmounts.length <= 32);
        uint256[] memory withdrawalAmounts = _fuzzWithdrawalAmounts(depositAmounts);
...
contract DelegationManagerUnitTests_completeQueuedWithdrawal is DelegationManagerUnitTests {
    function test_Revert_WhenExitWithdrawalQueuePaused() public {
        ...
        vm.expectRevert("Pausable: index is paused");
        delegationManager.completeQueuedWithdrawal(withdrawal, token);
    }

    function test_Revert_WhenInvalidWithdrawalRoot() public {
        _registerOperatorWithBaseDetails(defaultOperator);
        (IDelegationManager.Withdrawal memory withdrawal, IERC20 token, bytes32 withdrawalRoot) =
        _setUpCompleteQueuedWithdrawalSingleStrat({
            staker: defaultStaker,
            withdrawer: defaultStaker,
            depositAmount: 100,
        ...
        vm.roll(block.number + delegationManager.getWithdrawalDelay(withdrawal.strategies));
        vm.prank(defaultStaker);
        delegationManager.completeQueuedWithdrawal(withdrawal, token);
        assertFalse(
     ...
     */
    function test_Revert_WhenWithdrawalDelayBlocksNotPassed(uint256[] memory depositAmounts)
        public
    {
        vm.assume(depositAmounts.length > 0 && depositAmounts.length <= 32);
        uint256[] memory withdrawalAmounts = _fuzzWithdrawalAmounts(depositAmounts);
        ...
        delegationManager.completeQueuedWithdrawal(withdrawal, tokens[0]);
    ...
    function test_Revert_WhenTokensArrayLengthMismatch() public {
        ...
        vm.roll(block.number + delegationManager.getWithdrawalDelay(withdrawal.strategies));
        vm.expectRevert("DelegationManager._completeQueuedWithdrawal: input length mismatch");
        vm.prank(defaultStaker);
        delegationManager.completeQueuedWithdrawal(withdrawal, token);
    }
    ...
    function test_completeQueuedWithdrawal_SingleStratWithdrawAsTokens(
        ...
        vm.prank(staker);
        vm.expectEmit(true, true, true, true, address(delegationManager));
        emit WithdrawalCompleted(defaultOperator, staker, withdrawal.strategies[0], withdrawal.shares[0]);
        delegationManager.completeQueuedWithdrawal(withdrawal, token);
